package com.memvra.integration;
...
public class FactApiIntegrationTest {
    ...
    @Test
    void postingDuplicateReturnsConflict() {
        String base = "http://localhost:" + port;

        Map<String, Object> reqBody = Map.of(
                "content", "Dup test fact",
                "source_type", "user_input",
                "source_id", "itest:dup:1",
                "recorded_by", "itest-agent"
        );
        ...
        ResponseEntity<Map> first = rest.postForEntity(base + "/v1/facts", new HttpEntity<>(reqBody, headers), Map.class);
        assert first.getStatusCode().is2xxSuccessful();

        ResponseEntity<Map> second = rest.postForEntity(base + "/v1/facts", new HttpEntity<>(reqBody, headers), Map.class);
        assert second.getStatusCode().value() == 409;
        Map<?,?> err = second.getBody();
        assert err != null;
        assert "CONFLICT".equals(err.get("code"));
    }

    @Test
    void healthExposedAndInfoNotExposed() {
        String base = "http://localhost:" + port;
        ...
        Map<?,?> body = health.getBody();
        assert body != null && body.containsKey("status");

        ResponseEntity<String> info = rest.getForEntity(base + "/actuator/info", String.class);
        assert info.getStatusCode().value() == 404;
    }
}